...
public class GeminiService {
     ...
     */
    public String regenerateLessonNotes(String lessonTitle, String lessonContent, String courseTitle, String difficulty,
    ...
    public List<String> generateCourseRecommendations(String studentInterests, int estimatedIQ,
    ...
    private String buildQuizPrompt(String topic, String content, int numberOfQuestions) {
        return String.format("""
                ...
                """, numberOfQuestions, topic, content.length() > 1000 ? content.substring(0, 1000) + "..." : content,
                numberOfQuestions);
    }
    ...
    private String buildLessonNotesPrompt(String lessonTitle, String lessonContent, String courseTitle,
            String difficulty) {
        return String.format(
                """
    ...
    private String buildRegenerationPrompt(String lessonTitle, String lessonContent, String courseTitle,
            String difficulty, String previousNotes) {
        return String.format(
                """
    ...
    public Assignment generateCodingAssignment(String courseTitle, String topic, Assignment.Difficulty difficulty,
    ...
    private Quiz createFallbackIQTest() {
        ...
        q4.setId("iq-q4");
        q4.setQuestionText(
                "A cube has 6 faces. If you paint all faces red and then cut the cube into 27 smaller cubes, how many small cubes will have exactly 2 red faces?");
    ...
    private String buildCodingAssignmentPrompt(String courseTitle, String topic, Assignment.Difficulty difficulty,
            String programmingLanguage) {
        return String.format(
                "Generate a LeetCode-style coding assignment for the course '%s' on topic '%s' with %s difficulty in %s.\n\n"
                        +
                        "You MUST respond with ONLY a valid JSON object. Do not include any markdown formatting, backticks, or explanatory text outside the JSON.\n"
                        +
                        "The JSON structure must be exactly as follows:\n" +
                        "{\n" +
                        "  \"title\": \"Assignment Title\",\n" +
                        "  \"description\": \"Brief description\",\n" +
                        "  \"problemStatement\": \"Detailed problem statement in Markdown format. Use headers, bold text, lists, and code blocks where appropriate.\",\n"
                        +
    ...
    private Assignment parseCodingAssignmentResponse(String response, String topic, Assignment.Difficulty difficulty,
            String programmingLanguage) {
        try {
            ...
            assignment.setTitle(rootNode.path("title").asText("AI Generated: " + topic));
            assignment.setDescription(rootNode.path("description").asText("Coding challenge for " + topic));
            assignment.setProblemStatement(rootNode.path("problemStatement").asText());
            assignment.setConstraints(rootNode.path("constraints").asText());
            assignment.setStarterCode(rootNode.path("starterCode").asText());
            assignment.setSolution(rootNode.path("solution").asText());

            // Parse examples
            List<Assignment.Example> examples = new ArrayList<>();
            JsonNode examplesNode = rootNode.path("examples");
            if (examplesNode.isArray()) {
                for (JsonNode exampleNode : examplesNode) {
                    Assignment.Example example = new Assignment.Example();
                    example.setInput(exampleNode.path("input").asText());
                    example.setOutput(exampleNode.path("output").asText());
                    example.setExplanation(exampleNode.path("explanation").asText());
            ...
            // Parse test cases
            List<Assignment.TestCase> testCases = new ArrayList<>();
            JsonNode testCasesNode = rootNode.path("testCases");
            if (testCasesNode.isArray()) {
                for (JsonNode testCaseNode : testCasesNode) {
                    Assignment.TestCase testCase = new Assignment.TestCase();
                    testCase.setInput(testCaseNode.path("input").asText());
                    testCase.setExpectedOutput(testCaseNode.path("expectedOutput").asText());
                    testCase.setHidden(testCaseNode.path("isHidden").asBoolean(false));
